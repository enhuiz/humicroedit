...
class StanfordNLP:
    def __init__(self, host='http://localhost', port=9000):
        try:
            requests.get(host + ':{}'.format(port))
        except:
            raise Exception(
                "{}:{} is not open, "
                "please start stanford nlp server first: "
...
def pattern_match(D, posd1, pwi, centerverb, pattern,
                  n1=['NN', 'NNS', 'NNP', 'NNPS'],
    ...
    for dd in D:
        if dd[0] != 'ROOT':
            ...
            # first word pos match
            sel2 = np.where(
                np.any([posd1[:, 0] == pwi[dd[1]][1], posd1[:, 0] == pwi[dd[2]][1]]))
            # second word pos match
            sel3 = np.where(
                np.any([posd1[:, 2] == pwi[dd[2]][1], posd1[:, 2] == pwi[dd[1]][1]]))
            selind = np.intersect1d(sel1, np.intersect1d(sel2, sel3))
            if selind.size != 0:
                dd2append = [[dd[1], pwi[dd[1]][0], pwi[dd[1]][1]],
                             dd[0], [dd[2], pwi[dd[2]][0], pwi[dd[2]][1]]]
                E.append(dd2append)
                ...
                if selitem[0] in n1:  # the first word is noun
                    ...
                    if selitem[2] in n1:  # the second word is noun
                        ...
                        rmind = np.intersect1d(
                            rmind1, np.intersect1d(rmind2, rmind3))
...
def create_posd():
    ...
    # n1-nsubj-(v1-iobj-n2)-dobj-n3
    posd['s-v-o-o'] = np.asarray(posd2 + posd8 + posd9)
    posd['s-be-a'] = np.asarray(posd1 + posd10)  # n1-nsubj-a1-cop-be
    # n1-nsubj-v1-xcomp-a1-cop-be
    posd['s-v-be-a'] = np.asarray(posd2 + posd6 + posd10)
    # n1-nsubj-v1-xcomp-n2-cop-be
    posd['s-v-be-o'] = np.asarray(posd2 + posd7 + posd11)
    # n1-nsubj-v1-xcomp-v2-dobj-n2
    posd['s-v-v-o'] = np.asarray(posd2 + posd5 + posd4)
    posd['s-v-v'] = np.asarray(posd2 + posd5)  # n1-nsubj-v1-xcomp-v2
    # (n1-nsubj-a1-cop-be)-nmod-n2-case-p1
    posd['s-be-a-p-o'] = np.asarray(posd1 + posd10 + posd12 + posd14)
    # n1-nsubj-v1-nmod-n2-case-p1
    posd['s-v-p-o'] = np.asarray(posd2 + posd12 + posd14)
    # (n1-nsubj-v1-dobj-n2)-nmod-n3-case-p1
    posd['s-v-o-p-o'] = np.asarray(posd2 + posd4 + posd13 + posd14)
    posd['spass-v'] = np.asarray(posd3)  # n1-nsubjpass-v1
    # n1-nsubjpass-v1-nmod-n2-case-p1
    posd['spass-v-p-o'] = np.asarray(posd3 + posd12 + posd14)
...
def parse_events(text):
    ...
    index2token = {i + 1: w for i, w in enumerate(tokens)}
    dp = sNLP.dependency_parse(text)
    centerverb = index2token[dp[0][2]]
    pwi = {i + 1: w for i, w in enumerate(pos)}

    # events: [(pattern, flag, details)]
    events = [(
        pattern,
        *pattern_match(dp, posd[pattern], pwi, centerverb, pattern),
    ) for pattern in posd]

    events = filter(lambda t: t[1], events)
    events = map(lambda t: (t[0], t[2]), events)
    events = [*events]
