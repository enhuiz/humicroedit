...
def extract_skeleton(row, original=True):
    if original:
        events = row['original_event']
        text = extract_original(row['text'])
        term = row['text'].split('<swap1>')[1].split('<swap2>')[0].strip()
    else:
        events = row['edited_event']
        text = extract_edited(row['text'])
        term = row['text'].split('<swap2>')[1].split('<swap3>')[0].strip()
    events = eval(events)
    events = [sorted(set([(*entry,)
                          for entries in event[1]
                          for entry in entries
                          if isinstance(entry, list)]),
                     key=lambda t: t[0])
              for event in events]
    candidates = [' '.join([item[1] for item in event]) for event in events]
    candidates = [candidate for candidate in candidates if term in candidate]
    candidates = iter(sorted(candidates, key=len, reverse=True))
    skeleton = next(candidates, text)
    return skeleton
...
def atomic_generate(args, split):
    ...
    out_path = in_path.replace('.csv', '.kga.csv')
    ...
    for i, row in tqdm.tqdm(list(df.iterrows())):
        # original sentence
        original = extract_skeleton(row, True)
        ...
        # edited sentence
        edited = extract_skeleton(row, False)
